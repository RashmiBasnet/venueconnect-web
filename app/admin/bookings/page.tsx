...
function badge() {
    return "rounded-full bg-black/5 px-2.5 py-1 text-xs text-black/70";
}
...
export default async function AdminBookingsPage({
    ...
    const res = await handleGetAllBookings({ page, size, search });
                ...
                >
                    <div className="relative w-full">
                        ...
                        <input
                            name="search"
                            defaultValue={search}
                            placeholder="Search bookings (venue, user, etc.)"
            ...
            <div className="mt-4 overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm">
                <div className="flex items-center justify-between border-b border-black/10 px-5 py-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-semibold text-black">Bookings</h2>
                    </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-black/2 text-xs text-black/60">
                        <tr>
                            <th className="px-5 py-3">Venue / Date</th>
                    ...
                    <tbody className="divide-y divide-black/10">
                        {bookings.length ? (
                            bookings.map((b: any) => (
                                <tr key={String(b?._id)} className="hover:bg-black/2">
                                    ...
                                    <td className="px-5 py-3 text-black/80">{b?.guests ?? "—"}</td>
                                    <td className="px-5 py-3">
                                        <span className={badge()}>{b?.status ?? "—"}</span>
                                    </td>
                                    <td className="px-5 py-3">
                                        <span className={badge()}>{b?.paymentStatus ?? "—"}</span>
                                    </td>
